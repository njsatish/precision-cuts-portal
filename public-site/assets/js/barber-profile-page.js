...
function familyGallerySection(b){
  const gallery=b.familyGallery;
  const items=Array.isArray(gallery?.items)?gallery.items:[];
  if(!items.length)return "";
  const cards=items.map((item,index)=>`<button class="pc-family-card" type="button" data-family-src="${esc(item.src)}" data-family-index="${index}" aria-label="Enlarge ${esc(item.title||`Personal photo ${index+1}`)}"><img src="${esc(item.src)}" alt="${esc(item.alt||`Personal photo ${index+1}`)}" loading="lazy"><span class="pc-family-overlay"></span><span class="pc-family-copy"><strong>${esc(item.title||`Personal Photo ${index+1}`)}</strong><small>${esc(item.caption||"")}</small></span></button>`).join("");
  return `<section class="pc-family-section"><header class="pc-section-head"><h2>${esc(gallery.heading||"Life Beyond the Chair")}</h2><p class="pc-section-subtitle">${esc(gallery.description||"")}</p></header><div class="pc-family-grid">${cards}</div></section>`;
}
...
document.querySelectorAll('[data-family-src]').forEach(card=>card.addEventListener('click',()=>{
  full.src=card.dataset.familySrc;
  full.alt=card.querySelector('img')?.alt||'Expanded personal photo';
  captionCategory.textContent='Personal Gallery';
  captionTitle.textContent=card.querySelector('.pc-family-copy strong')?.textContent||'Personal Photo';
