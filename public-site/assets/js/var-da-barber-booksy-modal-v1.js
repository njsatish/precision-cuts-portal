(() => {
  ...
  const isLevarBookingTrigger = element => {
  ...
  const ensureModal = () => {
    let modal = document.getElementById("levar-booksy-modal-v1");
    if (modal) return modal;

    modal = document.createElement("div");
    modal.id = "levar-booksy-modal-v1";
    modal.className = "levar-booksy-modal-v1";
    modal.setAttribute("role", "dialog");
    modal.setAttribute("aria-modal", "true");
    modal.setAttribute("aria-labelledby", "levar-booksy-title-v1");
    modal.innerHTML = `
      <div class="levar-booksy-backdrop-v1" data-levar-booksy-close></div>
      <div class="levar-booksy-dialog-v1">
        <header>
          <div>
            <p>Book with ${BARBER_NAME}</p>
            <h2 id="levar-booksy-title-v1">Official Booksy Instant Schedule</h2>
          </div>
          <button type="button" data-levar-booksy-close aria-label="Close booking">Close</button>
        </header>
    ...
    iframe.addEventListener("load", () => {
      modal.querySelector(".levar-booksy-loader-v1")?.classList.add("is-hidden");
      iframe.classList.add("is-loaded");
    });
    modal.querySelectorAll("[data-levar-booksy-close]").forEach(element => {
  ...
  const openModal = event => {
    ...
    document.body.classList.add("levar-booksy-open-v1");
    modal.querySelector("header button").focus();
  };

  function closeModal() {
    const modal = document.getElementById("levar-booksy-modal-v1");
    if (!modal) return;
    modal.classList.remove("is-open");
    document.body.classList.remove("levar-booksy-open-v1");
    lastTrigger?.focus();
  }
  ...
  document.addEventListener("click", event => {
    ...
    if (!isLevarBookingTrigger(trigger)) return;
    openModal(event);
  }, true);
  ...
  const normalizeTriggers = () => {
    document.querySelectorAll("a, button, input[type='button'], input[type='submit']").forEach(trigger => {
      ...
      trigger.classList.add("book-with-levar-v1");
    });
  };
