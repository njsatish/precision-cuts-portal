(() => {
  ...
  const findCard = () => {
    const heading = [...document.querySelectorAll("main h1, main h2, main h3, main h4")]
      .find(node => node.textContent.trim().toLowerCase() === "christopher meadows");
  ...
  const apply = () => {
    ...
    card.classList.add("pc-about-christopher-card-v1");

    const image = card.querySelector("img");
    if (!image) return false;
    image.src = PHOTO;
    image.alt = "Christopher Meadows profile photo";
    image.removeAttribute("srcset");
    image.removeAttribute("data-src");
    image.classList.add("pc-about-christopher-photo-v1");

    const existingImageLink = image.closest("a");
    if (existingImageLink) {
      existingImageLink.href = PROFILE;
    } else {
      const link = document.createElement("a");
      link.href = PROFILE;
      link.className = "pc-about-christopher-photo-link-v1";
    ...
    if (!action || action.tagName !== "A") {
      ...
      replacement.className = action?.className || "pc-about-christopher-profile-v1";
      if (action) action.replaceWith(replacement);
      else card.appendChild(replacement);
      action = replacement;
    }
    action.href = PROFILE;
    action.textContent = "View Christopher Meadows’ profile";
    action.classList.add("pc-about-christopher-profile-v1");
    return true;
  };
