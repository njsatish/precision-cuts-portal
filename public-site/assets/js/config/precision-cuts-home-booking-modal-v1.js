(() => {
  ...
  function buildModal(){
    ...
    modal.hidden=true;
    modal.innerHTML=`<div class="pchbm-backdrop" data-pchbm-close></div><section class="pchbm-dialog" role="dialog" aria-modal="true" aria-labelledby="pchbm-title"><button class="pchbm-close" type="button" data-pchbm-close aria-label="Close booking">×</button><header><p>PRECISION CUTS ROANOKE</p><h2 id="pchbm-title">Book Your Chair</h2><span>Select your barber, service, and a live appointment time.</span></header><form id="pchbm-form"><label class="pchbm-wide"><span>Barber</span><select id="pchbm-barber" required></select></label><label class="pchbm-wide"><span>Service</span><select id="pchbm-service" required disabled><option>Choose a barber first</option></select></label><label><span>Date</span><select id="pchbm-date" required disabled><option>Choose a service first</option></select></label><label><span>Time</span><select id="pchbm-time" required disabled><option>Choose a date first</option></select></label><label><span>Full name</span><input id="pchbm-name" type="text" autocomplete="name" placeholder="John Doe" required></label><label><span>Phone number</span><input id="pchbm-phone" type="tel" autocomplete="tel" placeholder="(540) 555-0199" required></label><p id="pchbm-status" class="pchbm-status" aria-live="polite"></p><button id="pchbm-submit" class="pchbm-submit pchbm-wide" type="submit" disabled>Continue securely with Booksy</button></form></section>`;
    ...
    modal.querySelector("#pchbm-form").addEventListener("submit",submit);
    return modal;
  }
  ...
  async function open(barberId=""){
    ...
    populateBarbers(barberId);modal.hidden=false;document.documentElement.classList.add("pchbm-open");modal.querySelector(".pchbm-close").focus();
  }
