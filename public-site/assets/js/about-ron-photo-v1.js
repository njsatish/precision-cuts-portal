(() => {
  ...
  const findCard = () => {
    const heading = [...document.querySelectorAll("main h1, main h2, main h3, main h4")]
      .find(node => node.textContent.trim().toLowerCase() === "ron the barber");
  ...
  const apply = () => {
    ...
    card.classList.add("pc-about-ron-card-v1");

    const image = card.querySelector("img");
    if (!image) return false;
    image.src = PHOTO;
    image.alt = "Ron The Barber profile photo";
    image.removeAttribute("srcset");
    image.removeAttribute("data-src");
    image.classList.add("pc-about-ron-photo-v1");
    ...
    if (existingImageLink) existingImageLink.href = PROFILE;
      ...
      link.href = PROFILE;
      link.className = "pc-about-ron-photo-link-v1";
    ...
    if (!action || action.tagName !== "A") {
      ...
      replacement.className = action?.className || "pc-about-ron-profile-v1";
      if (action) action.replaceWith(replacement);
      else card.appendChild(replacement);
      action = replacement;
    }
    action.href = PROFILE;
    action.textContent = "View Ron The Barber’s profile";
    action.classList.add("pc-about-ron-profile-v1");
    return true;
  };
